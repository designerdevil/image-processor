...
class JumpToHyperspace {
    ...
    bind = () => {
        const el = document.getElementById("spaceJump");
        el.addEventListener('mousedown', this.initiate)
        el.addEventListener('touchstart', this.initiate)
        el.addEventListener('mouseup', this.enter)
        el.addEventListener('touchend', this.enter)
    }
    setup = () => {
